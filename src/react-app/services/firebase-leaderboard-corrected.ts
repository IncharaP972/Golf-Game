...
export class FirebaseLeaderboardService {
  ...
  static addScoreToLocal(playerName: string, level: number, strokes: number): void {
    // Fallback method for unauthenticated users
    const entry: FirebaseLeaderboardEntry = {
      uid: 'local',
    ...
    localStorage.setItem('localLeaderboard', JSON.stringify(existingEntries));
  }

  static getLocalEntries(): FirebaseLeaderboardEntry[] {
